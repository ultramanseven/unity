...
namespace NeoFPS.Samples
{
	public class OptionsMenuGraphics : OptionsMenuPanel
	{
        ...
        private void OnResolutionSettingsChanged()
        {
            ...
            int min = Mathf.FloorToInt(35f * m_AspectRatio * 5f / 5f);
            int max = Mathf.CeilToInt(67.5f * m_AspectRatio * 5f / 5f);

            // Change the slider
            m_FovSlider.SetLimits(min, max);
            m_FovSlider.value = Mathf.RoundToInt(Camera.VerticalToHorizontalFieldOfView(FpsSettings.graphics.verticalFoV, m_AspectRatio));
        }

        private void OnHorizontalFoVChanged(int fov)
        {
            FpsSettings.graphics.verticalFoV = Camera.HorizontalToVerticalFieldOfView(fov, m_AspectRatio);
        }
