...
#region INSTANTIATION
        ...
        public T InstantiatePrefab<T>(T prototype) where T : Component
        {
            ...
            if (SaveGameManager.enabled)
                result = NeoSerializedObjectFactory.Instantiate(prototype, m_SceneObjects);
            else
        ...
        public T InstantiatePrefab<T>(T prototype, Vector3 position, Quaternion rotation) where T : Component
        {
            ...
            if (SaveGameManager.enabled)
                result = NeoSerializedObjectFactory.Instantiate(prototype, m_SceneObjects, position, rotation);
            else
                result = Instantiate(prototype, position, rotation);
