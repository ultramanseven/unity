    ...
    public abstract class MotionGraphConnectableEditor : Editor
    {
        ...
        void DrawConnectionsListElements(Rect rect, int index, bool isActive, bool isFocused)
        {
            ...
            else
            {
                ...
                if (con.destination.parent == con.source)
                    EditorGUI.LabelField(labelRect, string.Format("this->{0} (child)", con.destination.name));
                else
                    EditorGUI.LabelField(labelRect, string.Format("this->{0} (sibling)", con.destination.name));
