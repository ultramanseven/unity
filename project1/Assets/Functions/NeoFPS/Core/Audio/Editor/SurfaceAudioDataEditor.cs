...
namespace NeoFPSEditor
{
	...
	public class SurfaceAudioDataEditor : Editor
	{
		...
		void CheckSurfaceConstantLength()
        {
			int targetCount = FpsSurfaceMaterial.count;

			// Check if size matches constants
			if( m_SurfacesArrayProperty.arraySize != targetCount)
            {
				// Shrink to match constants
				if (m_SurfacesArrayProperty.arraySize > targetCount)
					m_SurfacesArrayProperty.arraySize = targetCount;

				// Grow to match constants
				if (m_SurfacesArrayProperty.arraySize < targetCount)
				{
					++m_SurfacesArrayProperty.arraySize;
					var element = m_SurfacesArrayProperty.GetArrayElementAtIndex(m_SurfacesArrayProperty.arraySize - 1);
					element.FindPropertyRelative("clips").arraySize = 0;
