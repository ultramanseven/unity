...
namespace NeoFPS
{
    public class BodyLeanBase : MonoBehaviour, IAdditiveTransform, INeoSerializableComponent
    {
        ...
        protected Quaternion bodyPivotRotation
        {
            get;
            private set;
        }
        ...
        protected virtual void ApplyLean()
        {
            ...
            bodyPivotRotation = Quaternion.Euler(0f, 0f, m_MaxLeanAngle * m_CurrentLean);
            // Counter tilt
            if (m_WeaponTilt < 0.99f)
                m_LeanRotation = Quaternion.Slerp(Quaternion.identity, bodyPivotRotation, m_WeaponTilt);
            else
                m_LeanRotation = bodyPivotRotation;
        }
            ...
            public Quaternion rotation
            {
                get
                {
                    if (m_BodyLean != null && m_BodyLean.currentLean != 0f)
                        return Quaternion.Inverse(m_BodyLean.m_LeanRotation) * Quaternion.Slerp(m_BodyLean.bodyPivotRotation, Quaternion.identity, m_BodyLean.m_HeadCounterTilt);
                    else
