...
namespace NeoFPS
{
    ...
    public class ExplosiveObject : MonoBehaviour, IDamageHandler, INeoSerializableComponent
    {
        ...
        {
            return AddDamageInternal(damage, m_LocalTransform.position, null);
        }

        public virtual DamageResult AddDamage(float damage, RaycastHit hit)
        {
            return AddDamageInternal(damage, hit.point, null);
        }

        public virtual DamageResult AddDamage(float damage, IDamageSource source)
        {
            return AddDamageInternal(damage, m_LocalTransform.position, source);
        }

        public virtual DamageResult AddDamage(float damage, RaycastHit hit, IDamageSource source)
        {
            return AddDamageInternal(damage, hit.point, source);
        }

        DamageResult AddDamageInternal(float damage, Vector3 hitPoint, IDamageSource source)
        {
            ...
            if (m_Health > 0f)
            {
                ...
                // Report damage dealt
                DamageEvents.ReportDamageHandlerHit(this, source, hitPoint, DamageResult.Standard, damage);
