...
namespace NeoFPS
{
    ...
    public class PooledExplosion : MonoBehaviour, IDamageSource, INeoSerializableComponent
    {
        ...
        private static Dictionary<Transform, int> s_RootTransfomCounts = new Dictionary<Transform, int>();
        ...
        public virtual void Explode(float maxDamage, float maxForce, IDamageSource source = null, Transform ignoreRoot = null)
        {
            ...
            {
                var t = s_DetectedDamageHandlers[i].rootTransform;
                if (s_RootTransfomCounts.TryGetValue(t, out int count))
                    s_RootTransfomCounts[t] = count + 1;
                else
                    s_RootTransfomCounts[t] = 1;
            }

            // Calculate damage share
            for (int i = 0; i < s_DetectedDamageHandlers.Count; ++i)
            {
                var ddh = s_DetectedDamageHandlers[i];
                if (s_RootTransfomCounts.TryGetValue(ddh.rootTransform, out int count))
                    ddh.damageShare = 1f / (float)count;
                else
                    ddh.damageShare = 1f;

                s_DetectedDamageHandlers[i] = ddh;
            }
        ...
        protected virtual void CheckCollider(Collider c, Vector3 explosionCenter)
        {
            ...
            Transform rootTransform = c.transform;
            ...
            if (gatherDamangeHandlers)
            {
                ...
                for (int i = 0; i < s_DamageHandlers.Count; ++i)
                {
                    ...
                    for (int j = 0; j < s_DetectedDamageHandlers.Count; ++j)
                    {
                        ...
                        if (s_DetectedDamageHandlers[j].damageHandler == damageHandler)
                        {
                            if (s_DetectedDamageHandlers[j].falloff < falloff)
                                s_DetectedDamageHandlers[j] = new DamageHandlerInfo { damageHandler = damageHandler, falloff = falloff, rootTransform = s_DetectedDamageHandlers[j].rootTransform };
                    ...
                    if (damageHandler != null)
                    {
                        ...
                        if (s_DamageHandlers[i] is IHasHealthManager hasHealth)
                            rootTransform = hasHealth.healthTransform;

                        s_DetectedDamageHandlers.Add(new DamageHandlerInfo { damageHandler = damageHandler, falloff = falloff, rootTransform = rootTransform });
                    }
                }
                ...
                if (index == -1)
                {
                    ...
                    if (impactHandler != null || (rigidbody != null))// && !rigidbody.isKinematic))
                    {
                        ...
                        s_DetectedImpactHandlers.Add(new ImpactHandlerInfo { collider = c, impactHandler = impactHandler, falloff = falloff, direction = forceDirection });
                    }
                }
            }
        }
