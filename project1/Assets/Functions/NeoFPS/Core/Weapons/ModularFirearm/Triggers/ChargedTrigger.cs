                ...
                if (m_Charge != value)
                {
                    ...
                    switch (m_ChargeAnimation)
                    {
                        case ChargeAnimation.LayerWeight:
                            {
                                if (firearm.animationHandler.isValid)
                                    firearm.animationHandler.SetLayerWeight(m_LayerIndex, m_Charge);
                            }
                            break;
                        case ChargeAnimation.FloatParameter:
                            {
                                if ( m_ChargeAnimHash != 0 && firearm.animationHandler.isValid)
                                    firearm.animationHandler.SetFloat(m_ChargeAnimHash, m_Charge);
                            }
                            break;
                    }
