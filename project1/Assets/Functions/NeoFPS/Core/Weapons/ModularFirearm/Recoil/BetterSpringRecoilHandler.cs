...
namespace NeoFPS.ModularFirearms
{
    ...
    {
        [SerializeField, Tooltip("The additive transform handler which manages the firearm spring effects. If this is not set then it will be grabbed from the child hierarchy in awake.")]
        private AdditiveTransformHandler m_AdditiveTransform = null;
        ...
        protected override void Awake()
        {
            ...
            if (m_AdditiveTransform == null)
                m_AdditiveTransform = firearm.GetComponentInChildren<AdditiveTransformHandler>();
            if (m_AdditiveTransform != null)
            {
                m_WeaponRecoilEffect = m_AdditiveTransform.gameObject.GetComponent<FirearmRecoilEffect>();
                if (m_WeaponRecoilEffect == null)
                    m_WeaponRecoilEffect = m_AdditiveTransform.gameObject.AddComponent<FirearmRecoilEffect>();
            ...
            {
                Debug.LogError("Can't use the spring recoil handler without an AdditiveTransformHandler");
                enabled = false;
            }
        }
        ...
        public override void SetRecoilMultiplier(float move, float rotation)
        {
            ...
            // Apply to weapon
            m_AdditiveTransform.springPositionMultiplier = move;
            m_AdditiveTransform.springRotationMultiplier = rotation;
