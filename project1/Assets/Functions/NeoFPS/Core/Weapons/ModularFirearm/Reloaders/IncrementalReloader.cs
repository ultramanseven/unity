...
namespace NeoFPS.ModularFirearms
{
    ...
    public class IncrementalReloader : BaseReloaderBehaviour
    {
        ...
        private float m_ReloadTimeMultiplier = 1f;
        ...
        void ReloadPartialInternal (int count)
        {
            ...
            if (reloadCount == 0 || !firearm.ammo.available)
            {
                ...
                if (m_ReloadAudioEnd != null)
                    Invoke("PlayReloadEndSound", m_ReloadTimeout + m_ReloadIncrementDuration * m_ReloadTimeMultiplier);

                if (!m_UseExternalTriggers)
                    m_ReloadTimeout += m_ReloadEndDuration * m_ReloadTimeMultiplier;
            }
            else
            {
                if (!m_UseExternalTriggers)
                    m_ReloadTimeout += m_ReloadIncrementDuration * m_ReloadTimeMultiplier;
            }
        }
        ...
        public override Waitable Reload ()
		{
			if (!full && firearm.ammo.available)
            {
                ...
                {
                    m_ReloadTimeMultiplier = GetReloadDurationMultiplier();
                    m_ReloadTimeout = m_ReloadStartDuration * m_ReloadTimeMultiplier;
                }
