...
namespace NeoFPSEditor
{
    public static class NeoFpsEditorGUI
    {
        ...
        public static void Header(Rect r, string header)
        {
            EditorGUI.LabelField(r, header, EditorStyles.boldLabel);
        }
        ...
        #region VALUE MAX PAIR

        public static void IntValueMaxPair(SerializedProperty value, SerializedProperty max)
        {
            // Draw value field
            EditorGUI.BeginChangeCheck();
            ...
            if (EditorGUI.EndChangeCheck())
            {
                if (max.intValue < value.intValue)
                    value.intValue = max.intValue;
            }
        }

        public static void FloatValueMaxPair(SerializedProperty value, SerializedProperty max)
        {
            // Draw value field
            EditorGUI.BeginChangeCheck();
            EditorGUILayout.PropertyField(value);
            if (EditorGUI.EndChangeCheck())
            {
                if (value.floatValue > max.floatValue)
                    max.floatValue = value.floatValue;
            }
            ...
            if (EditorGUI.EndChangeCheck())
            {
                if (max.floatValue < value.floatValue)
        ...
        {
            if (!NeoFpsEditorPrefs.disableNeoObjectBrowsers)
            {
                if (root == null)
                {
                    prop.objectReferenceValue = null;
        ...
        public static int LayoutTabs(string[] options, SerializedProperty indexProperty)
        {
            int old = indexProperty.intValue;

            EditorGUILayout.Space();

            int selected = GUILayout.Toolbar(old, options, GUILayout.ExpandHeight(true));

            EditorGUILayout.Space();

            if (selected != old)
            {
                indexProperty.intValue = selected;
        ...
        public static int LayoutTabs(GUIContent[] options, SerializedProperty indexProperty)
        {
            int old = indexProperty.intValue;
        ...
        public static int LayoutTabs(Texture[] options, SerializedProperty indexProperty)
        {
            int old = indexProperty.intValue;
        ...
        #region ABOVE/BELOW NOTE

        private const string k_UpArrow = "\u25b2";
        private const string k_DownArrow = "\u25bc";

        private static GUIStyle s_InstructionsNoteStyle = null;
        public static GUIStyle instructionsNoteStyle
        {
            get
            {
                if (s_InstructionsNoteStyle == null)
                {
                    s_InstructionsNoteStyle = new GUIStyle(EditorStyles.wordWrappedMiniLabel) {
                        alignment = TextAnchor.MiddleCenter,
                        fontStyle = FontStyle.Bold
                    };
                }
                return s_InstructionsNoteStyle;
            }
        }
        public static void InstructionsNote(GUIContent note, bool up = true)
        {
            var arrow = up ? k_UpArrow : k_DownArrow;
            
            EditorGUILayout.BeginHorizontal();
            EditorGUILayout.Space();
            EditorGUILayout.LabelField(arrow, instructionsNoteStyle, GUILayout.Width(20));
            EditorGUILayout.LabelField(note, instructionsNoteStyle);
            EditorGUILayout.LabelField(arrow, instructionsNoteStyle, GUILayout.Width(20));
            EditorGUILayout.Space();
            EditorGUILayout.EndHorizontal();
            EditorGUILayout.Space();
